...
func _draw() -> void:
	...
	match data.id:
		"garlic":
			# 大蒜：绿色毒云光环（与飞刀/魔杖的弹体视觉完全区分）
			var r = 80.0 * effective_area()
			draw_circle(Vector2.ZERO, r, Color(0.30, 0.85, 0.35, 0.10))
			draw_circle(Vector2.ZERO, r * 0.66, Color(0.45, 0.95, 0.45, 0.10))
			draw_arc(Vector2.ZERO, r, 0, TAU, 48, Color(0.55, 1.0, 0.55, 0.45), 2.0)
		"bible":
			# 圣经：蓝色书页（小矩形 + 高光），环绕旋转
			var off = -global_position
			for c in orbit_children:
				var p = c.global_position + off
				draw_rect(Rect2(p - Vector2(7, 9), Vector2(14, 18)), Color(0.35, 0.7, 1.0, 0.9))
				draw_rect(Rect2(p - Vector2(4, 6), Vector2(8, 12)), Color(0.8, 0.92, 1.0, 0.95))
		"whip":
			# 鞭子：红色新月弧刃（横扫），与圣经的蓝色书页区分
			var off = -global_position
			for c in orbit_children:
				var p = c.global_position + off
				draw_arc(p, 17.0, orbit_angle, orbit_angle + PI * 0.85, 20, Color(1.0, 0.45, 0.3, 0.95), 4.0)
				draw_circle(p, 4.0, Color(1.0, 0.7, 0.4, 0.9))
		_:
			# 兜底（含 orbit 类其它武器）：旧蓝色圆逻辑
			var off = -global_position
			for c in orbit_children:
				draw_circle(c.global_position + off, 14.0, Color(0.4, 0.8, 1.0, 0.85))
